...
EXTI_HandleTypeDef hpb_exti[BUTTONn] = {{.Line = EXTI_LINE_5}};
...
static uint32_t IsUart8MspCbValid = 0;
#endif
__weak HAL_StatusTypeDef MX_UART8_Init(UART_HandleTypeDef* huart);
/**
 * @}
 */
...
static void UART8_MspInit(UART_HandleTypeDef *huart);
static void UART8_MspDeInit(UART_HandleTypeDef *huart);
#endif
/**
...
int32_t BSP_COM_Init(COM_TypeDef COM)
{
  ...
  else
  {
    ...
    /* Init the UART Msp */
    UART8_MspInit(&hcom_uart[COM]);
#else
    if(IsUart8MspCbValid == 0U)
    {
...
#endif
    if (MX_UART8_Init(&hcom_uart[COM]))
    {
      ret = BSP_ERROR_PERIPH_FAILURE;
    }
  }
...
__weak HAL_StatusTypeDef MX_UART8_Init(UART_HandleTypeDef* huart)
{
  ...
  huart->Instance = BUS_UART8_INSTANCE;
...
#if (USE_HAL_UART_REGISTER_CALLBACKS == 1U)
/**
 * @brief Register Default UART8 Bus Msp Callbacks
 * @retval BSP status
 */
int32_t BSP_COM_RegisterDefaultMspCallbacks(COM_TypeDef COM)
{
  ...
  else
  {
    ...
    if(HAL_UART_RegisterCallback(&hcom_uart[COM], HAL_UART_MSPINIT_CB_ID, UART8_MspInit) != HAL_OK)
    {
      ret = BSP_ERROR_PERIPH_FAILURE;
    }
    else if(HAL_UART_RegisterCallback(&hcom_uart[COM], HAL_UART_MSPDEINIT_CB_ID, UART8_MspDeInit) != HAL_OK)
    {
      ret = BSP_ERROR_PERIPH_FAILURE;
    }
    else
    {
      IsUart8MspCbValid = 1U;
    }
  }
...
/**
 * @brief Register UART8 Bus Msp Callback registering
 * @param Callbacks pointer to UART8 MspInit/MspDeInit callback functions
 * @retval BSP status
 */
...
#endif /* USE_COM_LOG */
/**
 * @brief  Initializes UART8 MSP.
 * @param  huart UART8 handle
 * @retval None
 */

static void UART8_MspInit(UART_HandleTypeDef* uartHandle)
{
  ...
  */
    ...
    PE1     ------> UART8_TX
    */
    GPIO_InitStruct.Pin = BUS_UART8_RX_GPIO_PIN;
    GPIO_InitStruct.Mode = GPIO_MODE_AF_PP;
    GPIO_InitStruct.Pull = GPIO_NOPULL;
    GPIO_InitStruct.Speed = GPIO_SPEED_FREQ_VERY_HIGH;
    GPIO_InitStruct.Alternate = BUS_UART8_RX_GPIO_AF;
    HAL_GPIO_Init(BUS_UART8_RX_GPIO_PORT, &GPIO_InitStruct);

    GPIO_InitStruct.Pin = BUS_UART8_TX_GPIO_PIN;
    GPIO_InitStruct.Mode = GPIO_MODE_AF_PP;
    GPIO_InitStruct.Pull = GPIO_NOPULL;
    GPIO_InitStruct.Speed = GPIO_SPEED_FREQ_VERY_HIGH;
    GPIO_InitStruct.Alternate = BUS_UART8_TX_GPIO_AF;
    HAL_GPIO_Init(BUS_UART8_TX_GPIO_PORT, &GPIO_InitStruct);
...
static void UART8_MspDeInit(UART_HandleTypeDef* uartHandle)
{
  ...
  /* USER CODE END UART8_MspDeInit 0 */
    ...
    PE1     ------> UART8_TX
    */
    HAL_GPIO_DeInit(BUS_UART8_RX_GPIO_PORT, BUS_UART8_RX_GPIO_PIN);

    HAL_GPIO_DeInit(BUS_UART8_TX_GPIO_PORT, BUS_UART8_TX_GPIO_PIN);
